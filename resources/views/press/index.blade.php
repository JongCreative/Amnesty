...
{{-- <div class="press_container">
    <div class="press_card">
        ...
        <div class="card_description">Pietjes Puk is al 25 jaar beeldredacteur bij het NRC. Pietje heeft heel veel verstand van foto's. In 1530 was hij
            de eerste winnaar van de zilveren camera</div>
    ...
    <div class="content_positioning">
        <article class="content_wrapper">
            ...
            @foreach($press as $pressArticle)
                <div class="press_card">
                    <div class="card_name">{{ $pressArticle->title }}</div>
                    <div class="card_date">{{ date('d-m-Y', strtotime($pressArticle->date)) }}</div>
                    <div class="card_description">{!! nl2br(e( $pressArticle->descr))!!}</div>
                    <div class="card_link"><a href="{{ $pressArticle->link }}">View article</a></div>
                    @guest @else @if (Auth::user()->admin)
...
{{--
                    ...
                    <div><img src="https://pbs.twimg.com/profile_images/931190512731807746/yO8Lcr9H_400x400.jpg" alt="{{ $pressArticle->source }}'s logo"
                        /></div> --}}